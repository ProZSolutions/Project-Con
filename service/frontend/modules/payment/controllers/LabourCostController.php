...
class LabourCostController extends \yii\web\Controller
{
  ...
  public function actionIndex() {         
    $query= new Query;      
    $query ->from('labour_cost')      
    ->select("`id`, `labour_id`, `cost`, `start_date`, `end_date`");           
  ...
  public function actionUpdateLabourCost($id) {
    $params = Yii::$app->getRequest()->getBodyParams();  
    $model = $this->findModel($id);  
    $model->attributes=$params;
    if($model->save()) {      
  ...
  public function actionDeleteLabourCost($id) {
    $model=$this->findModel($id);      
    if($model->delete()) { 
      $this->setHeader(200);
  ...
  protected function findModel($id) { 
    if (($model = LabourCost::findOne($id)) !== null) {
      return $model;
    }
    else {
      $this->setHeader(400);
