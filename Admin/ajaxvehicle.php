...
?>
 <?php while($rs = mysqli_fetch_object($result)){ 
	 ?>
     <form action="vehicle_attend.php"method="POST">   	
	  <input type="hidden" name="id11"value="<?php echo stripslashes($rs->nos); ?> ">
	
	<label class="auto-style1">Vehicle No</label><input type="text" name="id12"value="<?php echo stripslashes($rs->vno); ?>" class="auto-style2" style="width: 221px"><br><br>
	<label class="auto-style1">Vehicle Name</label><input type="text"name="id13"value="<?php echo stripslashes($rs->vown);?>" class="auto-style3" style="width: 221px"><br><br>
	<?php } ?>
