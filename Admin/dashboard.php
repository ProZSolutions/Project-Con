...
<!--Payment Details-Bar-->
                  <?php  $cd=date('Y-m-d',strtotime("-6 day"));
...
<!---Attendance Details-->
<?php
$rp= "select count(*) from `vehicle_entry`"; 
$rp1=mysqli_query($link,$rp);
			while($dg=mysqli_fetch_assoc($rp1))
					{
$po1=$dg['count(*)'];
...
$rp= "select count(*) from `labour_attend`"; 
$rp1=mysqli_query($link,$rp);
while($dg=mysqli_fetch_assoc($rp1))
					{
$po2=$dg['count(*)'];
...
$bfg= "SELECT count(*) FROM `contact` WHERE `Designation`='Labour'";
$rp2=mysqli_query($link,$bfg);
			while($dg2=mysqli_fetch_assoc($rp2))
					{
$hds=$dg2['count(*)'];
...
<?php require("Template/body.php"); ?>
       ...
       <!-- page content -->
		...
		<?php  
			  ...
			  <!-- <div class="count" id="prevpay"></div>
  -->             <span class="count_bottom"><i class="green"><i class="fa fa-calendar-check-o"></i></i> From last Week</span>
            ...
            <div class="col-md-2 col-sm-4 col-xs-6 tile_stats_count">
			 ...
			 <?php  
              ...
              <!-- <div class="count" id="weekload"></div> -->
              <span class="count_bottom"><i class="green"><i class="fa fa-calendar-check-o"></i></i> From last Week</span>
            </div>
            <div class="col-md-2 col-sm-4 col-xs-6 tile_stats_count">
              <span class="count_top"><i class="fa fa-truck"></i> Total Loads</span>
			  <?php  
				 $date1=date("W");
                 $current=$date1;
				$ps= "SELECT count(id) FROM `material_in` WHERE week(Date)=$current";
				$dg=mysqli_query($link,$ps);
				while($d=mysqli_fetch_assoc($dg))
					{
					$sk=$d['count(id)'];?>
...
<script>
    ...
    <script>
      $(document).ready(function() {
          ...
          colors: ['#8762D6','#15E2DD','#F6A1C3','#BAF752','#26B99A', '#34495E', '#ACADAC', '#3498DB'],
        ...
        $MENU_TOGGLE.on('click', function() {
		 ...
		 Morris.Bar({
          ...
          xkey: 'x',
          ykeys: ['y', 'z', 'a','p'],
          barColors: ['#8762D6','#15E2DD','#F6A1C3','#BAF752','#E67A7F','#A9CFA6','#BCE882','#007E6A','#9B59B6', '#3c321E','#CF495A', '#59B66C', '#B6596C'],
          hideHover: 'auto',
          labels: ['Employee', 'Vehicle', 'Labour','Ot'],
          resize: true
	...
	<script>
      ...
      echartPieCollapse.setOption({
        ...
        legend: {
          x: 'center',
          y: 'bottom',
          data: ['Employee', 'Labour', 'Driver', 'Dealer']
        },
        toolbox: {
          show: true,
          feature: {
